...
export default function LayananPage() {
  ...
  return (
    <div className="p-8 bg-white min-h-screen">
      {/* HEADER UTAMA & GARIS PEMISAH (Persis Berita) */}
      <div className="flex justify-between items-center border-b-2 border-purple-500 pb-2 mb-8">
        <h1 className="text-blue-600 font-bold text-xl italic">Layanan</h1>
        <button
          onClick={() => {
      ...
      {/* SUB-HEADER (Persis seperti "Klinik") */}
      <h2 className="text-blue-800 italic font-semibold mb-6">
        Daftar Layanan
      </h2>
          ...
          <form action={handleSubmit} className="space-y-6 max-w-5xl">
            <div className="flex items-center gap-4">
              ...
              <input
                ...
                type="text"
                required
                className="flex-1 border-2 border-blue-400 rounded-xl p-2 outline-none"
              />
            </div>

            <div className="flex items-center gap-4">
              <label className="w-24 text-gray-700 font-medium">Harga :</label>
              <input
                name="harga"
            ...
            <div className="flex items-center gap-4">
              <label className="w-24 text-gray-700 font-medium">Jadwal :</label>
              <input
                name="jadwal"
            ...
            <div className="flex items-start gap-4">
              <label className="w-24 text-gray-700 mt-2 font-medium">
                Ket :
              </label>
              <textarea
                name="keterangan"
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <input
                  ...
                  accept="image/*"
                  required={!editData}
                  className="text-sm file:mr-4 file:py-2 file:px-6 file:rounded-full file:border-0 file:bg-blue-900 file:text-white file:font-bold cursor-pointer"
                />
                {editData && (
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="bg-blue-900 text-white px-12 py-3 rounded-full font-bold shadow-lg hover:opacity-90 active:scale-95 transition-all"
              >
                {isUploading
                  ? "Memproses..."
                  : editData
      ...
      {/* GRID DAFTAR LAYANAN (Persis Berita) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {listLayanan.length > 0 ? (
          listLayanan.map((item) => (
            <div
              key={item.id}
              className="group relative aspect-square bg-blue-900 rounded-[2.5rem] overflow-hidden shadow-md transition-transform hover:scale-105"
            >
              {item.gambar && (
                <img
                  src={item.gambar}
